...
void BPF_STRUCT_OPS(arca_enqueue, struct task_struct *p, u64 enq_flags)
{
    u32 pid = p->pid;
    enum arca_task_class *cls = bpf_map_lookup_elem(&task_class_map, &pid);
    enum arca_task_class class_val = cls ? *cls : ARCA_CLASS_UNKNOWN;

    u64 slice = get_slice(class_val);

    /* initialize vtime for new tasks */
    if (p->scx.dsq_vtime == 0)
        p->scx.dsq_vtime = vtime_now;

    u64 vtime = p->scx.dsq_vtime;
    /* bound idle accumulation to one slice */
    if (time_before(vtime, vtime_now - slice))
        vtime = vtime_now - slice;

    scx_bpf_dsq_insert_vtime(p, SHARED_DSQ, slice, vtime, enq_flags);

    if (class_val == ARCA_CLASS_INTERACTIVE)      stat_inc(0);
    else if (class_val == ARCA_CLASS_CPU_BOUND)   stat_inc(1);
    else if (class_val == ARCA_CLASS_BATCH)       stat_inc(2);
}
...
{
    /* charge vtime for execution time, scaled by weight */
